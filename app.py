...
if page == "📊 Vue Générale":
    ...
    with col_l:
        if COL_DATE:
            ...
            fig = px.bar(ca_mois, x="Mois", y="CA (€)", title="📈 CA par mois", color_discrete_sequence=["#22C55E"])
            fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#F8FAFC", title_font_family="Syne", xaxis=dict(showgrid=False), yaxis=dict(gridcolor="#1E3A5F"))
    ...
    with col_r:
        ...
        fig2.update_layout(
            title="📋 Statut devis", paper_bgcolor="rgba(0,0,0,0)", font_color="#F8FAFC",
            title_font_family="Syne", showlegend=True, legend=dict(font=dict(color="#94A3B8")),
            annotations=[dict(text=f"<b>{int(nb_signes/nb_devis*100) if nb_devis else 0}%</b>", x=0.5, y=0.5, font_size=22, showarrow=False, font_color="#F8FAFC")]
        )
    ...
    cols = [c for c in [COL_CLIENT, COL_CHANTIER, COL_NUM_DEVIS, COL_MONTANT, COL_DATE, COL_RELANCE1, COL_STATUT] if c]
    ...
    st.divider()
    cols = [c for c in [COL_CLIENT, COL_CHANTIER, COL_MONTANT, COL_FACT_FIN, COL_MODALITE, COL_STATUT] if c]
    ...
    st.dataframe(df_f.drop(columns=["_montant","_signe","_fact_fin","_pv","_statut_ch"], errors="ignore"), use_container_width=True, hide_index=True)
